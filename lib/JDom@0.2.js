...
class JDom {
     ...
     * @param {[Object|string|JDom]} [node.children] The child node(s) of the element.
     * 
     */
    constructor(node) {
        if (node) {
        ...
        if (node.attr) {
            for (let key in node.attr) {
                if (key.substring(0, 5) === "data_") {
                    ...
                    // console.log(key, ele[key], this.idList);
                    // if (key === "id") {
    ...
    render(selector) {
        ...
        // EDIT [2025-08-25]: Guard against missing container selector - AA
        if (!container) {
    ...
    update(newContent, selector) {
        try {
            ...
            // EDIT [2025-08-25]: Fix error variable name; surface details - AA
            console.error(err);
        }
    ...
    #findCurrentElement(){
        if (this.node.attr && typeof(this.node.attr.id) !== "undefined") {
            ...
            // EDIT [2025-08-25]: Fix typo in error string - AA
            throw "an id must be assigned to this element."
        }
    }
